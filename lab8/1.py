...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.setGeometry(360, 250, 1000, 615)
    ...
    def _connect_to_db(self):
        self.conn = psycopg2.connect(database="rasp_bd",
                                     user="postgres",
    ...
    def _create_day_table(self, day):
        ...
        table.setHorizontalHeaderLabels(["id", "День", "Урок", "Аудитория", "Время", "Номер пары","Неделя", "", ""])
    ...
    def _update_day_table(self, table, day):
        ...
        for i, r in enumerate(records):
            ...
            ed_sql = "UPDATE timetable SET ned=%s, day=%s,  room_numb=%s, start_time=%s, num=%s WHERE id=%s"
            ...
            table.setItem(i, 4, QTableWidgetItem(str(r[4])))
            table.setItem(i, 5, QTableWidgetItem(str(r[5])))
            table.setItem(i, 6, QTableWidgetItem(str(r[6])))
            table.setCellWidget(i, 7, editButton)
            table.setCellWidget(i, 8, delButton)

            editButton.clicked.connect(
                lambda _, num=i, tabl=table, edit=True, sql=ed_sql, tab="timetable":
                self._change_from_table(num, tabl, edit, sql, tab))
            delButton.clicked.connect(
                lambda _, num=i, tabl=table, edit=False, sql=del_sql, tab="timetable":
                self._change_from_table(num, tabl, edit, sql, tab))

        addButton = QPushButton("Add")
        add_sql = "INSERT INTO timetable VALUES (%s, %s, %s, %s, %s, %s, %s)"
        addButton.clicked.connect(
            lambda _, num=len(records), tabl=table, sql=add_sql, tab="timetable": self._add_row(num, tabl, sql, tab))
    ...
    def _update_teachers_tab(self, table):
        self.cursor.execute(f"SELECT * FROM teacher ORDER BY id")
        records = list(self.cursor.fetchall())
        # print(records)
        table.setRowCount(len(records) + 1)
        ...
        for i, r in enumerate(records):
            ...
            r = list(r)
            editButton = QPushButton("Edit")
            ed_sql = "UPDATE teacher SET full_name=%s, subjectt=%s,  id=%s WHERE id=%s"
            delButton = QPushButton("Delete")
            ...
            editButton.clicked.connect(
                lambda _, num=i, tabl=table, edit=True, sql=ed_sql, tab="teacher":
                self._change_from_table(num, tabl, edit, sql, tab))
            delButton.clicked.connect(
                lambda _, num=i, tabl=table, edit=False, sql=del_sql, tab="teacher":
                self._change_from_table(num, tabl, edit, sql, tab))
        addButton = QPushButton("Add")
        add_sql = "INSERT INTO teacher VALUES (%s, %s, %s)"
        addButton.clicked.connect(
            lambda _, num=len(records), tabl=table, sql=add_sql, tab="teacher": self._add_row(num, tabl, sql, tab))
    ...
    def _update_subjects_tab(self, table):
        ...
        add_sql = "INSERT INTO subject VALUES (%s)"
        addButton = QPushButton("Add")
        addButton.clicked.connect(
            lambda _, num=len(records), tabl=table, sql=add_sql, tab='subjects': self._add_row(num, tabl, sql, tab))
    ...
    def _change_subjects(self, rowNum, table, old):
        ...
        for i in range(table.columnCount() - 2):
            try:
                ...
                row.append(None)
        try:
            self.cursor.execute(f"SELECT * FROM teacher WHERE subjectt=%s", (old,))
            records = list(self.cursor.fetchall())
            self.cursor.execute("DELETE FROM teacher WHERE subjectt=%s", (old,))
            self.cursor.execute("UPDATE subject SET name=%s WHERE name=%s", (row[0],old ))
            self.conn.commit()
            for e in records:
                r = list(e)
                r[2] = row[0]

                self.cursor.execute("INSERT INTO teacher VALUES (%s, %s, %s)", tuple(r))
    ...
    def _delete_subjects(self, rowNum, table):
        ...
        for i in range(table.columnCount() - 2):
            try:
                ...
                row.append(None)
        try:
            self.cursor.execute(f"SELECT * FROM teacher WHERE subjectt=%s", (row[0],))
            records = list(self.cursor.fetchall())
            #print('1')
            #for e in records:
                #self.cursor.execute("DELETE FROM timetable where teacher_id=%s", (e[0],))
            self.cursor.execute("DELETE FROM teacher where subjectt=%s", (row[0],))
            self.cursor.execute("DELETE FROM subject where name=%s", (row[0],))
            self.conn.commit()
